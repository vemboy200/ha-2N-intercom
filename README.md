...
- The doorbell binary sensor can also turn on when Control4 itself places a call *into* the door station (for example, a page-through/announcement), not only when a visitor presses the physical button — from the device's own perspective, both look like an incoming call. Turn off the **Ring on incoming call** (`ring_on_call`) option in the integration's options flow (Doorbell step) to stop this: the doorbell will then only ring on real `KeyPressed` button events (via **Ring on button press** / `ring_on_keypress`), while the separate Call state sensor keeps tracking calls either way.
